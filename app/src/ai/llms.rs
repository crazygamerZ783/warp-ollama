...
pub fn is_using_api_key_for_provider(provider: &LLMProvider, app: &AppContext) -> bool {
    ...
    match provider {
        ...
        LLMProvider::Ollama => false, // Ollama is local, no API key needed
        _ => false,
    }
}
...
impl Default for ModelsByFeature {
    fn default() -> Self {
        ...
        // Build Ollama model choices if enabled
        let ollama_choices: Vec<LLMInfo> = if ollama_config.enabled {
            vec![LLMInfo {
                display_name: format!("Ollama ({})", ollama_config.model),
                base_model_name: ollama_config.model.clone(),
                id: OllamaConfig::model_id(&ollama_config.model).into(),
                reasoning_level: None,
                usage_metadata: LLMUsageMetadata {
        ...
        // Choose the default: if Ollama is enabled, use it as default; otherwise "auto"
        let (default_agent_id, mut agent_choices) = if ollama_config.enabled {
            (
                OllamaConfig::model_id(&ollama_config.model),
                ollama_choices.clone(),
            )
        } else {
            (
                "auto".to_owned(),
        ...
        // Always include the standard "auto" choice as a fallback
        agent_choices.push(LLMInfo {
        ...
        Self {
            agent_mode: AvailableLLMs {
                default_id: default_agent_id.into(),
            ...
            coding: AvailableLLMs {
                ...
                choices: {
                    let mut c = ollama_choices.clone();
                    c.push(LLMInfo {
