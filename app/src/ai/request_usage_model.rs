...
impl AIRequestUsageModel {
    ...
    pub fn refresh_request_usage_async(&mut self, ctx: &mut ModelContext<Self>) {
        // Skip server-side quota check if Ollama is enabled (local inference is free)
    ...
    pub fn has_any_ai_remaining(&self, ctx: &AppContext) -> bool {
        ...
        has_base_plan_ai_requests
            ...
            || has_byo_api_key
            // Ollama is local and free — always available
            || crate::ai::ollama::config::OllamaConfig::global().enabled
    }
